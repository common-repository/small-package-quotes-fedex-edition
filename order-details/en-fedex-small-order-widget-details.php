...
if (!class_exists("En_Fedex_Small_Order_Widget_Details")) {

    class En_Fedex_Small_Order_Widget_Details
    {
        ...
        {
            $this->multi_ship_id = 'multi_fedex_small';
        ...
        function en_assign_order_details($actions)
        {
            ...
            if (!empty($this->result_details) && count($this->result_details) > 0) {
                ...
                add_meta_box('en_additional_order_details', __('Additional Order Details', 'woocommerce'), array($this, 'en_fedex_small_add_meta_box_order_widget'), 'shop_order', 'side', 'low', 'core');
            }
        ...
        function en_origin_services_details($order_data, $shipment_status, $ship_count, $single_price_details = array())
        {
            ...
            if ($shipment_status != 'single') {
                ...
                if (
                    ...
                    if (isset($service_order_data->cheapest_services->rate)) {
                        echo '<li>';
                        echo $title . ' ' . $resd . ' ' . $this->en_format_price($service_order_data->cheapest_services->rate);
                        echo '</li>';
                    } else {
                        echo '<li>';
                    ...
                    if (isset($service_order_data->cheapest_services->rate)) {
                        echo '<li>';
                        echo $resd . '  ' . $this->en_format_price($service_order_data->cheapest_services->rate);
                        echo '</li>';
                    } else {
            ...
            foreach ($service_order_data->items as $value) {
                ...
                $get_product_name = '';
                if ($product_name) {
                    $get_product_name = $product_name->get_name();
                } else {
                    $get_product_name = isset($value->productName) ? $value->productName : '';
                }
                echo '<li>' . $value->productQty . ' x ' . $get_product_name . '</li>';
            }
            echo '</ul>';
            echo "<br /><br />";
        }
        ...
        public function en_show_accessorials($service_order_data, $shipment_status)
        {
            ...
            $residential_del = get_option('fedex_small_quote_as_residential_delivery');
            /* Residential feature */
            if (
         ...
         * Add order details in metabox.
         */
        public function en_fedex_small_add_meta_box_order_widget()
        {
            $order_details = $this->get_formatted_meta_data;
            $this->en_fedex_small_origin_services_details($order_details);
        }
         ...
         */
        function en_fedex_small_origin_services_details($order_data)
        {
            $this->currency_symbol = get_woocommerce_currency_symbol(get_option('woocommerce_currency'));
            $this->count = 0;
            $en_account_details = [];
        ...
        public function show_order_widget_detail()
        {
            ...
            /* Show accessorials */
            $this->en_fedex_small_show_accessorials(array_unique(array_merge($this->accessorials, $this->label_sufex)));
         ...
         */
        public function en_fedex_small_show_accessorials($service_order_data)
        {
    ...
    new En_Fedex_Small_Order_Widget_Details();
}
