...
if (!class_exists('EnOrderWidget')) {
    ...
    class EnOrderWidget
    {
         ...
         * Show order details.
         */
        public function en_show_order_widget($number_shipment, $sender_origin, $label, $cost, $accessorials, $items, $en_extra_accessories, $discount = null)
        {
            ...
            if(!empty($discount)){
                echo '<li>Total discount : ' . esc_attr($this->en_format_price($discount)) . '</li>';
            }
            echo "</ul>";
            echo "<br>";
        ...
        public function en_order_widget()
        {
            ...
            foreach ($this->en_widget_details as $method_key => $en_widget_details) {
                ...
                foreach ($shipments as $count => $shipment) {
                    ...
                    $discount = '';
                    if(isset($rate['discount_services']) && !empty($rate['discount_services'])){
                        $discount = round($rate['discount_services'], 2);
                    }
                    $this->en_show_order_widget($number_shipment, $sender_origin, $label, $cost, $accessorials, $items_li, $en_extra_accessories, $discount);
