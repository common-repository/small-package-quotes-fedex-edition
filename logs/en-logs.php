...
if (!class_exists('EnFedexSpqLogs')) {
    class EnFedexSpqLogs
    {
        ...
        {
            $obj_classs = new \FedEx_Small_Curl_Request();
            $data = array(
                'serverName' => fedex_small_get_domain(),
                'licenseKey' => get_option('fedex_small_licence_key'),
                'lastLogs' => '25',
                'carrierName' => get_option('api_selection_fedex_small') == 'new_api' ? 'FedEx Small New API' : 'FedEx Small'
            );

            require_once('en-json-tree-view/en-jtv.php');
            require_once 'en-packaging-details/en-packaging-details.php';

            $url = FEDEX_SPQ_MAIN_DOMAIN . '/request-log/index.php';
            $logs = $obj_classs->fedex_small_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'fedex-small-log-' . $key . rand(1, 100);
                    ...
                    if (isset($temp_resp->binPackaging) && !empty($temp_resp->binPackaging)) {
                        ...
                        $bin_packaging = new \EnFedexSpqLogsPackaging();
